...
- **GUÍAS DE NORMALIZACIÓN GEOGRÁFICA (ARGENTINA/LATAM):**
  - Si el texto menciona "CABA", "Capital Federal" o "Buenos Aires" (ciudad), usa la jerarquía: Ciudad=Ciudad Autónoma de Buenos Aires, Provincia=CABA, País=Argentina.
  - Si menciona una provincia (ej. "Santa Fe", "Chaco"), identifícala como tal en el campo `provincia`.
  - Para ciudades importantes (Rosario, Córdoba, Mendoza, Mar del Plata), asegúrate de incluir la provincia correcta.
  - No inventes coordenadas si no las conoces con precisión, deja que el sistema de normalización interno las asigne.
...
SCHEMA REQUERIDO:
{
  ...
  "categoria_principal": "politica_nacional|economia|policiales|deportes|espectaculos|ciencia_tecnologia|sociedad|internacional|negocios|salud|turismo|otro",
  ...
  "temporal_intelligence": {
    ...
    "eventos_fechados": [
      {
        "id": 1,
        "descripcion": "string (mínimo 2 oraciones, muy descriptiva, con contexto suficiente para entenderse de forma aislada sin leer el artículo principal)",
...
- En geo_intelligence, solo geolocaliza lugares EXPLÍCITAMENTE nombrados en el texto. Para cada lugar, intenta identificar la jerarquía administrativa completa (ciudad, provincia/estado, país) si está presente o es deducible del contexto inmediato.
...
async def get_analysis_prompt() -> str:
    try:
        async with get_db_session() as conn:
            row = await conn.fetchrow("SELECT value FROM system_configs WHERE key = 'PROMPT_ANALYSIS'")
            if row and row['value']:
                return row['value']
    except Exception as e:
        print(f"Error fetching PROMPT_ANALYSIS from DB: {e}")
    return ANALYSIS_SYSTEM_PROMPT

async def get_crossmedia_prompt() -> str:
    try:
        async with get_db_session() as conn:
            row = await conn.fetchrow("SELECT value FROM system_configs WHERE key = 'PROMPT_CROSSMEDIA'")
            if row and row['value']:
                return row['value']
    except Exception as e:
        print(f"Error fetching PROMPT_CROSSMEDIA from DB: {e}")
    return CROSSMEDIA_COMPARISON_PROMPT
